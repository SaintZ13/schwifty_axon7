...
int zte_ts_pinctrl_init(ZTE_PINCTRL_INFO_T *pinctrl_node)
{
	...
	if (pinctrl_node->active_describe) {
		dev_err(pinctrl_node->dev, "active describe %s\n", pinctrl_node->active_describe);
		pinctrl_node->ts_state_active = pinctrl_lookup_state(pinctrl_node->ts_pinctrl,
						pinctrl_node->active_describe);
		if (IS_ERR(pinctrl_node->ts_state_active)) {
			dev_err(pinctrl_node->dev,
				"cannot get ts pinctrl active state\n");
			return PTR_ERR(pinctrl_node->ts_state_active);
		}
	}

	if (pinctrl_node->suspend_describe) {
		dev_err(pinctrl_node->dev, "suspend describe %s\n", pinctrl_node->suspend_describe);
		pinctrl_node->ts_state_suspend = pinctrl_lookup_state(pinctrl_node->ts_pinctrl,
						 pinctrl_node->suspend_describe);
		if (IS_ERR(pinctrl_node->ts_state_suspend)) {
			dev_err(pinctrl_node->dev,
				"cannot get ts pinctrl suspend state\n");
			return PTR_ERR(pinctrl_node->ts_state_suspend);
		}
	}

	if (pinctrl_node->release_describe) {
		dev_err(pinctrl_node->dev, "release describe %s\n", pinctrl_node->release_describe);
		pinctrl_node->ts_state_release = pinctrl_lookup_state(pinctrl_node->ts_pinctrl,
						 pinctrl_node->release_describe);
		if (IS_ERR(pinctrl_node->ts_state_release)) {
			dev_err(pinctrl_node->dev,
				"cannot get ts pinctrl release state\n");
			return PTR_ERR(pinctrl_node->ts_state_release);
		}
	}
