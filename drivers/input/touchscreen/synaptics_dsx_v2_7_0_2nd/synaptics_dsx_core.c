	...
	for (finger = 0; finger < fingers_to_process; finger++) {
		...
		case F12_GLOVED_FINGER_STATUS:
			...
			input_report_key(rmi4_data->input_dev,
					BTN_TOOL_FINGER, 1);
			input_report_abs(rmi4_data->input_dev,
					ABS_MT_POSITION_X, x + TOUCH_ABS_OFFSET_X);
			input_report_abs(rmi4_data->input_dev,
					ABS_MT_POSITION_Y, y);
#ifdef REPORT_2D_W
...
int synaptics_get_max_std_rawdata_2nd(short *max_rawdata)
{
	short *rawdata2_max[] = {
		...
		rawdata2_max12, rawdata2_max13, rawdata2_max14, rawdata2_max15, rawdata2_max16,
	};
...
int synaptics_get_min_std_rawdata_2nd(short *min_rawdata)
{
	...
	short *rawdata2_min[] = {
		...
		rawdata2_min12, rawdata2_min13, rawdata2_min14, rawdata2_min15, rawdata2_min16,
	};
	unsigned char num_of_rx = exp_data.rmi4_data->num_of_rx;
	unsigned char num_of_tx = exp_data.rmi4_data->num_of_tx;
...
static int synaptics_rmi4_probe(struct platform_device *pdev)
{
	...
	pinctrl_node.active_describe = "tlmm_ts_active";
	pinctrl_node.suspend_describe = "tlmm_ts_suspend";
	if (zte_ts_pinctrl_init(&pinctrl_node) != 0)
		goto err_init_pinctrl;
	zte_ts_pinctrl_configure(&pinctrl_node, pinctrl_node.ts_state_active);
	...
	if (evdata && evdata->data && rmi4_data) {
		if (event == FB_EVENT_BLANK) {
			...
			if (*transition == FB_BLANK_POWERDOWN) {
				if (!zte_ts_is_td4322()) {
					SYNA_INFO("fb later node=%d s3330 power down\n", fb_infos->node);
	...
	if (evdata && evdata->data && rmi4_data) {
		if (event == FB_EARLY_EVENT_BLANK) {
			...
			if (*transition == FB_BLANK_POWERDOWN) {
				...
				if (!zte_ts_is_td4322()) {
					SYNA_INFO("fb early node=%d s3330 power on\n", fb_infos->node);
					synaptics_rmi4_resume(&rmi4_data->pdev->dev);
				}
			}
		}
...
exit:
	...
	SYNA_INFO("%s: disable regulator\n", __func__);

	zte_ts_pinctrl_configure(&pinctrl_node, pinctrl_node.ts_state_suspend);
