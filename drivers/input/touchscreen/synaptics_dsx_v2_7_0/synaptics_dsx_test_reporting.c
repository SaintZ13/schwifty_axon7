...
static ssize_t synaptics_rmi4_raw_data_config(int report_type)
{
	...
	retval = test_sysfs_report_type_store(NULL, NULL, buf, sizeof(buf));
	if (retval < 0)
		goto exit;
...
static int synaptics_rawdata_test_read(struct seq_file *m, void *v)
{
	...
	retval = synaptics_get_max_std_rawdata(max_rawdata);
	if (retval < 0) {
	...
	retval = synaptics_get_min_std_rawdata(min_rawdata);
	if (retval < 0) {
...
static int synaptics_rmi4_test_init(struct synaptics_rmi4_data *rmi4_data)
{
	...
	refresh = proc_create("touchscreen/ts_rawdata_read", 0664,
					NULL, &rawdata_read_proc_ops);
	if (refresh == NULL) {
		SYNA_ERROR("proc_create ts_rawdata_read failed!\n");
	}

	refresh = proc_create("touchscreen/ts_rawdata_test", 0664,
					NULL, &rawdata_test_proc_ops);
	if (refresh == NULL) {
		SYNA_ERROR("proc_create ts_rawdata_test failed!\n");
	}

	refresh = proc_create("touchscreen/ts_short_circuit_test", 0664,
