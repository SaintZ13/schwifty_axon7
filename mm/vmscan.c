...
#ifdef CONFIG_ZTE_PRIO_APP /* head file */
...
#ifdef CONFIG_ZTE_PRIO_APP_CNTL /* tag PZPA_M008_001 */
	/* use the latest full calc for about 99/100 loop here */
	int zprio_keep = is_light_pzpa_zprio_lru_keep();
#endif

	referenced_ptes = page_referenced(page, 1, sc->target_mem_cgroup,
					  &vm_flags);
#ifdef CONFIG_ZTE_PRIO_APP_CNTL /* tag PZPA_M008 */
	/* try to active zprio page when enabled */
	if (zprio_keep &&
		PageZprio(page)) {
		referenced_ptes++;
		...
		isolate_mode_t mode, enum lru_list lru)
{
	...
	unsigned long scan;
#ifdef CONFIG_ZTE_PRIO_APP_CNTL /* tag PZPA_M007_001 */
	int zprio_keep = is_pzpa_zprio_lru_keep();
	int zprio_part_max =
		pzpa_get_zprio_lru_scan_factor();
	if (zprio_keep
		&& zprio_part_max <=
		PZPA_MAX_SCAN_MULTIPLE
		&& zprio_part_max > 0) {
		/* get the nr for zprio */
		zprio_part_max =
			nr_to_scan *
			(zprio_part_max - 1);
	} else
	...
	for (scan = 0; scan < nr_to_scan && !list_empty(src); scan++) {
...
#ifdef CONFIG_ZTE_PRIO_APP_CNTL /* tag PZPA_M007_002 */
		/* active zprio lru, and copy below case -EBUSY */
		if (zprio_keep &&
			PageZprio(page) &&
			page_lru(page) ==
			LRU_ACTIVE_FILE) {
		...
		case 0:
			...
			nr_taken += nr_pages;
#ifdef CONFIG_ZTE_PRIO_APP /* tag PZPA_M005_007 */
			/* no CONFIG_TRANSPARENT_HUGEPAGE */
			/* file page to active or inactive */
			pzpa_update_del_stats(
				NR_LRU_BASE + lru, page);
#endif
			break;
...
putback_inactive_pages(struct lruvec *lruvec, struct list_head *page_list)
{
	...
	while (!list_empty(page_list)) {
		...
		if (put_page_testzero(page)) {
			...
			del_page_from_lru_list(page, lruvec, lru);
#ifdef CONFIG_ZTE_PRIO_APP /* tag PZPA_M005 */
			pzpa_cc_zprio_flags(page);
#endif
				     ...
				     enum lru_list lru)
{
	...
	while (!list_empty(list)) {
		...
		pgmoved += nr_pages;
#ifdef CONFIG_ZTE_PRIO_APP /* tag PZPA_M004 */
		pzpa_update_add_stats(
			NR_LRU_BASE + lru, page);
#endif
...
#ifdef CONFIG_ZTE_PRIO_APP_CNTL /* tag PZPA_M006_001 */
	unsigned long zprio_factor;
#endif

	inactive = get_lru_size(lruvec, LRU_INACTIVE_FILE);
	active = get_lru_size(lruvec, LRU_ACTIVE_FILE);
#ifdef CONFIG_ZTE_PRIO_APP_CNTL /* tag PZPA_M006_001 */
	/* delay the balance by excluding some zprio pages */
	zprio_factor = pzpa_get_ifl_factor(PZPA_FULL_CALC);
	if (zprio_factor) {
		/* this is a inverse avg factor */
		active = active * (zprio_factor - 1) / zprio_factor;
		if (is_pzpa_zprio_lru_dbg_enabled())
			pr_info("pzpa ifl ret %ld, %ld\n",
				zprio_factor, active);
	} else if (is_pzpa_zprio_lru_dbg_enabled())
		pr_info("pzpa ifl check %ld, %ld\n",
			active, inactive);
#endif
